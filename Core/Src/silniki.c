...
//uint8_t jedz_sync(float kat_os1, float kat_os2)
//{
//    printf("[SYNC] OS1→%.1f°  OS2→%.1f°\r\n", kat_os1, kat_os2);
//
//    /* 1. Homing sekwencyjnie */
//  //  jedz_do_zderzaka_blok(PIN_GORA, 2);
//  //  jedz_do_zderzaka_blok(PIN_CCW,  1);
//
//    uint32_t t_os1 = (uint32_t)(kat_os1 * ms_per_deg_os1);
//    uint32_t t_os2 = (uint32_t)(kat_os2 * ms_per_deg_os2);
//
//    /* 2. Odpal obie jednocześnie */
//
//
//    stop_all();
//    HAL_Delay(30);
//    if (t_os1 > 0) HAL_GPIO_WritePin(GPIOA, PIN_DOL, GPIO_PIN_SET);
//    if (t_os2 > 0) HAL_GPIO_WritePin(GPIOA, PIN_CW,  GPIO_PIN_SET);
//
//    // start_kierunek(PIN_DOL);
//    // start_kierunek(PIN_CW);
//
//    uint32_t start   = HAL_GetTick();
//    uint32_t t_blank = start + BLANK_MS;
//    uint8_t  done1   = (t_os1 == 0);
...
//        uint32_t elapsed = now - start;
//
//        /* Zatrzymaj oś gdy czas minął */
//        if (!done1 && elapsed >= t_os1) {
//            HAL_GPIO_WritePin(GPIOA, PIN_DOL, GPIO_PIN_RESET);
//            done1 = 1;
//            printf("[SYNC] OS1 na %.1f°\r\n", kat_os1);
//        }
//        if (!done2 && elapsed >= t_os2) {
//            HAL_GPIO_WritePin(GPIOA, PIN_CW, GPIO_PIN_RESET);
//            done2 = 1;
//            printf("[SYNC] OS2 na %.1f°\r\n", kat_os2);
//        }
//
